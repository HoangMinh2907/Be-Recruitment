...
public class PostController : ControllerBase
{
    ...
    [HttpGet]
    public async Task<ActionResult<ResponsePostCompany>> PostByLocation(string location)
    {
        var post = await _postService.GetPostLocation(location);
        return post == null ? NotFound() : Ok(post);
    }

    [HttpGet]
    public async Task<ActionResult<ResponsePostCompany>> PostBySalary(double salary)
    {
        var post = await _postService.GetPostSalary(salary);
        return post == null ? NotFound() : Ok(post);
    }
